...
2. EMBEDDING (FastEmbed)
   → Dense: [0.12, -0.34, 0.56, ... (384 numbers)]
   → Sparse: {vacation: 0.8, policy: 0.7, ...}

3. QDRANT SEARCH
   → Dense search finds: chunks about "time off", "leave", "PTO"
   → Sparse search finds: chunks with exact word "vacation"
   → RRF combines: Both rankings merged
