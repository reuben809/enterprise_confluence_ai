...
The **Enterprise Confluence AI** is a Retrieval-Augmented Generation (RAG) system that enables intelligent question-answering over Confluence documentation. It combines state-of-the-art embedding models, hybrid vector search, and local LLM inference to provide accurate, context-aware responses.

### Key Features
- **Hybrid Search**: Dense (semantic) + Sparse (keyword) retrieval with RRF fusion
- **Offline-First**: All models run locally without internet dependency
- **Enterprise Ready**: Handles 30K+ document chunks efficiently
- **LM Studio Integration**: Uses any OpenAI-compatible local LLM
...
| 3 | MCP Server Integration | Q1 2025 |
| 3 | Remove MongoDB dependency | Q1 2025 |
| 3 | Agentic RAG with tool calling | Q1 2025 |
...
flowchart TB
    ...
    subgraph Intelligence Layer
        RET["HybridRetriever<br/>Dense + Sparse"]
        RRK["LocalReranker<br/>FlashRank"]
...
|-----------|------------|---------|
| **UI** | Streamlit | Web interface for Q&A |
| **API** | FastAPI | REST endpoints, streaming |
| **Embeddings** | FastEmbed ONNX | Dense + Sparse vectors |
| **Vector Store** | Qdrant | Hybrid search, RRF fusion |
| **LLM** | LM Studio | Local AI inference |
| **Reranker** | FlashRank | Cross-encoder scoring |

### 3.3 Data Flow Overview

1. **Ingestion**: Confluence → MongoDB → Chunking → FastEmbed → Qdrant
2. **Query**: User → API → Hybrid Search → Rerank → LLM → Response
...
class HybridRetriever:
    ...
    def search(self, query: str, limit: int = 10) -> List[Dict]:
        # 1. Generate dense embedding 384 dimensions
        # 2. Generate sparse embedding variable dimensions
        # 3. Prefetch from both indices
        # 4. Fuse with RRF
...
flowchart LR
    ...
    subgraph Embedding
        DE["Dense Model bge-small-en-v1.5"]
        SE["Sparse Model Splade_PP_en_v1"]
    end
...
| ms-marco-TinyBERT | ~50 MB | ~100 MB | HuggingFace |
